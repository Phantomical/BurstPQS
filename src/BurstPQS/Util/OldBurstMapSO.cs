...
using UnityEngine;
using HeightAlpha = BurstPQS.Util.BurstMapSO.HeightAlpha;

namespace BurstPQS.Util;

public struct OldBurstMapSO : IBurstMapSO
{
    ...
    private OldBurstMapSO(MapSO mapSO, ReadOnlyMemorySpan<byte> data)
    {
        Width = mapSO.Width;
        Height = mapSO.Height;
    ...
    /// </para>
    /// </remarks>
    public static unsafe MapSOGuard Create(MapSO mapSO, out OldBurstMapSO burst)
    {
    ...
    public readonly Color32 GetPixelColor32(float x, float y)
    {
        var c = ConstructBilinearCoords(x, y);
        return Color32.Lerp(
            Color32.Lerp(GetPixelColor32(c.minX, c.minY), GetPixelColor32(c.maxX, c.minY), c.midX),
            Color32.Lerp(GetPixelColor32(c.minX, c.maxY), GetPixelColor32(c.maxX, c.maxY), c.midX),
            c.midY
        );
    }
