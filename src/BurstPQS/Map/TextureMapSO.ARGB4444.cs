...
public static partial class TextureMapSO
{
    ...
    public struct ARGB4444 : IMapSO
    {
        ...
        public ARGB4444(NativeArray<byte> data, int width, int height, MapSO.MapDepth depth)
        {
            int required = width * height * 2;
            if (data.Length < required)
                throw new ArgumentException(
                    $"Data length {data.Length} is too small for {width}x{height} ARGB4444 texture (need at least {required})",
                    nameof(data)
                );
            this.data = data;
            Width = width;
