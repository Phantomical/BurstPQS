...
{
    [BurstCompile]
    public struct RGBAFloat : IMapSO
    {
        ...
        public RGBAFloat(Texture2D texture, MapSO.MapDepth depth)
        {
            ValidateFormat(texture, TextureFormat.RGBAFloat);
            this = new RGBAFloat(
        ...
        public RGBAFloat(NativeArray<float> data, int width, int height, MapSO.MapDepth depth)
        {
            int required = width * height * 4;
            if (data.Length < required)
                throw new ArgumentException(
                    $"Data length {data.Length} is too small for {width}x{height} RGBAFloat texture (need at least {required})",
                    nameof(data)
                );
            this.data = data;
            Width = width;
