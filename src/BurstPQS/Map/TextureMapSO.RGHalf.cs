...
public static partial class TextureMapSO
{
    ...
    public struct RGHalf : IMapSO
    {
        ...
        public RGHalf(Texture2D texture, MapSO.MapDepth depth)
        {
            ValidateFormat(texture, TextureFormat.RGHalf);
            this = new RGHalf(
        ...
        public RGHalf(NativeArray<half> data, int width, int height, MapSO.MapDepth depth)
        {
            int required = width * height * 2;
            if (data.Length < required)
                throw new ArgumentException(
                    $"Data length {data.Length} is too small for {width}x{height} RGHalf texture (need at least {required})",
                    nameof(data)
                );
            this.data = data;
            Width = width;
