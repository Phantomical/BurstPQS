...
public static partial class TextureMapSO
{
    ...
    public struct ARGB32 : IMapSO
    {
        ...
        public ARGB32(Texture2D texture, MapSO.MapDepth depth)
        {
            ValidateFormat(texture, TextureFormat.ARGB32);
            this = new ARGB32(
        ...
        public ARGB32(NativeArray<byte> data, int width, int height, MapSO.MapDepth depth)
        {
            int required = width * height * 4;
            if (data.Length < required)
                throw new ArgumentException(
                    $"Data length {data.Length} is too small for {width}x{height} ARGB32 texture (need at least {required})",
                    nameof(data)
                );
            this.data = data;
            Width = width;
        ...
        )
        {
            ...
            a = data[i] * Byte2Float;
            r = data[i + 1] * Byte2Float;
            g = data[i + 2] * Byte2Float;
            b = data[i + 3] * Byte2Float;
        }
